...
const Accueil = ({ isAuthenticated }) => {
  ...
  const fetchMemes = async () => {
    try {
      ...
      const apiUrl = isAuthenticated
        ? "http://localhost:5000/api/m" // Authentifié
        : "http://localhost:5000/api/memes"; // Non authentifié
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const apiUrl = isAuthenticated
        ? "http://localhost:5000/api/memes" // Après auth
        : "http://localhost:5000/api/memes/test"; // Avant auth
